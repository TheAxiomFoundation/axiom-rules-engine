            ...
            if current >= kinds.len() || related >= kinds.len() {
                    ...
                    && filtered_entity != Some(entity)
                {
                    self.push(
                        ...
                        citing,
                        format!(
                            "`{citing}` evaluates on `{entity}` ids and aggregates relation `{relation}` keyed on slot {current}, which declares `{current_kind}` (slot kinds {}); the lookup can never match an `{entity}` id. Declare the kinds in tuple order and recompile",
                            format_kinds(kinds)
                        ),
                    );
                }
